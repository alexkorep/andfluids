...
char* GeomX::AddElem ( uchar b, href& ndx )
{
	if ( mBuf[b].num >= mBuf[b].max ) {
		if ( long(mBuf[b].max) * 2 > ELEM_MAX ) {
			error.PrintF ( (char*)"geom", (char*)"Maximum number of elements reached.\n" );
			error.Exit ();
		}
		mBuf[b].max *= 2;		
		char* new_data = (char*) malloc ( mBuf[b].max * mBuf[b].stride );
...
hpos GeomX::HeapExpand ( ushort size, ushort& ret  )
{
	...
	if ( mHeapMax > HEAP_MAX ) {
		error.PrintF ( (char*)"geom", (char*)"Geom heap size exceeds range of index.\n" );
		error.Exit ();
	}
	hval* pNewHeap = (hval*) malloc ( mHeapMax * sizeof(hval) );
	if ( pNewHeap == 0x0 ) {
		error.PrintF ( (char*)"geom", (char*)"Geom heap out of memory.\n" );
		error.Exit ();
	}
	...
	if ( pos < mHeapFree || mHeapFree == -1 ) {
		...
		* (hpos*) (mHeap+pos + FPOS) = mHeapFree;
		...
		if ( mHeapFree != -1 && *(hpos*) (mHeap+mHeapFree+FPOS) == 0x0 ) {
			error.PrintF ( (char*)"geomx", (char*)"ERROR: Heap pointer 0. pHeapFree, %d\n", pos );
			error.Exit ();
		}
		...
		if ( pCurr !=mHeap-1 && *(hpos*) (pCurr+FPOS) == 0x0 ) {
			error.PrintF ( (char*)"geomx", (char*)"ERROR: Heap pointer 0. pCurr, %d, %d\n", x, pos );
			error.Exit ();
		}
		if ( pPrev !=mHeap-1 && *(hpos*) (pPrev+FPOS) == 0x0 ) { error.PrintF ( (char*)"geomx", (char*)"ERROR: Heap pointer 0. pPrev, %d, %d\n", x, pos ); error.Exit (); }
		if ( pNext !=mHeap-1 && *(hpos*) (pNext+FPOS) == 0x0 ) { error.PrintF ( (char*)"geomx", (char*)"ERROR: Heap pointer 0. pNext, %d, %d\n", x, pos ); error.Exit (); }
		if ( *(hpos*) (mHeap+mHeapFree+FPOS) == 0x0 ) { error.PrintF ( (char*)"geomx", (char*)"ERROR: Heap pointer 0. pHeapFree, %d, %d\n", x, pos ); error.Exit (); }
		...
		while ( pNext != mHeap-1 ) {
			...
			if ( pNext < pPrev && pNext != mHeap-1 ) {
				error.PrintF ( (char*)"geomx", (char*)"ERROR: Heap free space out of order. %d, %d\n", x, pos );
				error.Exit ();
			}
		}
		//---
